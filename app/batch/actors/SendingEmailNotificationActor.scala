...
import models.task.States.Complete
...
class SendingEmailNotificationActor(
  ...
  val intervalTask: Cancellable =
    context.system.scheduler.schedule(5 seconds, 10 minutes, self, CheckTasks)
  ...
  private[this] def checkTasks: Unit = {
    ...
    (for {
      tasks <- taskRepository.find(now, tomorrow).map(_.filter(_.state != Complete).groupBy(_.author))
